...
public class MeshGenerator : MonoBehaviour {
    ...
    Vector2 regionSize = new Vector2(750f, 750f);
    ...
    public void Run () {
        ...
        if (fixedMapSize) {
            ...
            if (Application.isPlaying) {
                InitVisibleChunks ();
                parentChunks = GameObject.FindGameObjectsWithTag("veg");
                foreach (GameObject veg in parentChunks)
                {
                    DeleteOldTrees(veg);
                }
            }
    ...
    public void DeleteOldTrees(GameObject vegchunk)
    {
        Mesh vegchunkmesh = vegchunk.GetComponent<MeshFilter>().sharedMesh;
                        ...
                        if (IsVisibleFrom (bounds, Camera.main)) {
                            if (recycleableChunks.Count > 0) {
                                ...
                                if(coord.y == 0)
                                {
                                    ...
                                    Chunk grassChunk = CreateGrassChunk (coord, chunk);
                                    grassChunk.coord = coord;
                                    grassChunk.SetUp(grassmat, false);
                                    grassChunk.tag = "grassChunk";
    ...
    public void UpdateChunkMesh (Chunk chunk) {
        ...
        Transform child = chunk.transform.Find("GrassChunk");
        Mesh grassmesh = child.GetComponent<MeshFilter>().sharedMesh;
        ...
        grassmesh.triangles = meshTriangles;
    ...
    void InitChunks () {
        ...
        for (int i = 0; i < oldChunks.Count; i++) {
            Transform child = oldChunks[i].transform.Find("GrassChunk");
    ...
    Chunk CreateChunk (Vector3Int coord) {
        GameObject chunkinstance = new GameObject ($"Chunk ({coord.x}, {coord.y}, {coord.z})");
        
        chunkinstance.transform.parent = chunkHolder.transform;
        Chunk newChunk = chunkinstance.AddComponent<Chunk> ();
        newChunk.coord = coord;
        
        vegetation.GetComponent<Vegetation>().PlaceTreesRandomly(newChunk, Tree1, Tree2, Tree3, Tree4, 200, 400, 12f);
        return newChunk;
    }

    Chunk CreateGrassChunk (Vector3Int coord, Chunk parentChunk) {
        GameObject chunkinstance = new GameObject ("GrassChunk");
        
        chunkinstance.transform.parent = parentChunk.transform;
        Chunk newChunk = chunkinstance.AddComponent<Chunk> ();
        newChunk.coord = coord;
        newChunk.tag = "grassChunk";
